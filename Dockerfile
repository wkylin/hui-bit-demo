...
ARG SCOPE_PATH=/root/hui-bit
WORKDIR ${SCOPE_PATH}
RUN bit init --bare
CMD bit start


# $ docker build -f ./Dockerfile -t bit-demo/bit-server .
# $ docker run -it -v persisted:/root/hui-bit -p 3000:3000 bit-demo/bit-server:latest
